...
class PowersmsSendWizard(osv.osv_memory):
    ...
    def _get_accounts(self, cr, uid, context=None):
        ...
        if template.enforce_from_account:
            return [
                (
                    template.enforce_from_account.id,
                    "%s (%s)"
                    % (template.enforce_from_account.name, template.enforce_from_account.tel_id),
                )
            ]
        else:
    ...
    def get_value(self, cursor, user, template, message, context=None, id=None):
        """
        Gets the value of the message parsed with the content of object id
        (or the first 'src_rec_ids' if id is not given)
        """
    ...
    def _get_template(self, cr, uid, context=None):
        ...
        if "template_id" in context.keys():
            template_ids = template_obj.search(
                cr, uid, [("id", "=", context["template_id"])], context=context
            )
        if not template_ids:
            return None
    ...
    def _get_template_value(self, cr, uid, field, context=None):
        ...
        if not template:
            return False
        if (
            len(context["src_rec_ids"]) > 1
        ):  # Multiple Mail: Gets original template values for multiple email change
    ...
    def _get_template_to_value(self, cr, uid, field, context=None):
        ...
        if (
            ...
            return ", ".join(
                [
                    self.get_value(cr, uid, template, getattr(template, field), context, id)
    ...
    def create_report_attachment(
        ...
        if template.report_template:
            ...
            data = {}
            data["model"] = ir_model_obj.browse(
                cr, uid, screen_vals["rel_model"], context=context
            ).model
            ...
            attach_vals = {
                ...
                "datas_fname": tools.ustr(
                    self.get_end_value(cr, uid, src_rec_id, reportname, template, context=context)
                    or _("Report")
                )
                + "."
                + format,
    ...
    def save_to_smsbox(self, cr, uid, ids, context=None):
        ...
        for rec_id in report_record_ids:
            ...
            phone_numbers = self.get_value(
                cr, uid, template, getattr(template, "def_to"), context, rec_id
            )
            ...
            for number in psms_to_list:
                ...
                # If not found, user's launguage is used.
                ctx = context.copy()
                if template.lang:
                    ...
                    if len(
                        self.pool.get("res.lang").search(
                            cr, uid, [("name", "=", lang)], context=context
                        )
                    ):
                        ctx["lang"] = lang
                if not ctx.get("lang", False) or ctx["lang"] == "False":
                    ctx["lang"] = self.pool.get("res.users").read(
                        cr, uid, uid, ["context_lang"], context
                    )["context_lang"]
                ...
                if attachment_ids:
                    mailbox_vals = {"pem_attachments_ids": [[6, 0, attachment_ids]]}
    ...
    _defaults = {
        "rel_model": lambda self, cr, uid, ctx: self.pool.get("ir.model").search(
            cr, uid, [("model", "=", ctx["src_model"])], context=ctx
        )[0],
        "to": lambda self, cr, uid, ctx: self.pool.get("powersms.core_accounts").filter_send_sms(
            cr, uid, self._get_template_to_value(cr, uid, "def_to", ctx)
        ),
        "from": lambda self, cr, uid, ctx: self._get_template_value(cr, uid, "def_from", ctx),
        "body_text": lambda self, cr, uid, ctx: self._get_template_value(
            cr, uid, "def_body_text", ctx
        ),
