...
class PowersmsTemplates(osv.osv):
    ...
    def _get_model_name(self, cursor, uid, template_ids, field_name, arg, context=None):
        ...
        for template_id in template_ids:
            model_id = pwm_templ_obj.read(cursor, uid, template_id, ["object_name"])["object_name"]
            if not model_id:
                res[template_id] = False
                continue
            mod_name = self.pool.get("ir.model").read(cursor, uid, model_id[0], ["model"], context)[
                "model"
            ]
    ...
    def create_action_reference(self, cursor, uid, ids, context):
        template = self.pool.get("powersms.templates").browse(cursor, uid, ids[0])
        ...
        if not template.ref_ir_act_window:
            ...
            ref_ir_act_window = action_obj.create(
                ...
                uid,
                {
                    ...
                    "view_id": self.pool.get("ir.ui.view").search(
                        cursor, uid, [("name", "=", "powersms.send.wizard.form")], context=context
                    )[0],
                    "target": "new",
                    "auto_refresh": 1,
                },
            ...
            ref_ir_value = values_obj.create(
                ...
                uid,
                {
                    ...
                    "model": src_obj,
                    "key2": "client_action_multi",
                    "value": "ir.actions.act_window," + str(template.ref_ir_act_window),
                    "object": True,
                },
                context,
            )
    ...
    def get_value(self, cursor, user, recid, message=None, template=None, context=None):
        ...
        if message:
            try:
                ...
                object = self.pool.get(template.object_name.model).browse(cursor, user, recid, ctx)
                env = context.copy()
                env.update(
                    {
                        "user": self.pool.get("res.users").browse(cursor, user, user, context),
                        "db": cursor.dbname,
                    }
                )
    ...
    _columns = {
        ...
        "def_from": fields.char("Remitent (From)", size=250, help="Public name of remitent"),
        "def_to": fields.char(
            "Recepients (To)",
            size=250,
            help="The recepient(s) of sms. " "Placeholders can be used here.",
        ),
        ...
        "ref_ir_act_window": fields.many2one(
            "ir.actions.act_window", "Window Action", readonly=True
        ),
        "certificate": fields.boolean(
            "Use certificated SMS", help="The SMS will be sent certificated"
        ),
        "lang": fields.char(
            "Language",
            size=250,
        ...
        "use_filter": fields.boolean(
            "Active Filter",
            help="This option allow you to add a custom python filter" " before sending a sms",
        ),
        "send_on_create": fields.boolean(
            "Send on Create", help="Sends a SMS when a new document is created."
        ),
        "send_on_write": fields.boolean(
            "Send on Update", help="Sends a SMS when a document is modified."
        ),
        "model_int_name": fields.function(
            _get_model_name, string="Model Internal Name", type="char", size=250, method=True
        ),
    ...
    _sql_constraints = [("name", "unique (name)", _("The template name must be unique!"))]
